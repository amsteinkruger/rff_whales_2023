...
target = 
  bind_rows(tibble(target = seq(5.3, 8.3, by = 0.10)) %>% mutate(inf = "i"), 
            tibble(target = seq(3.7, 6.1, by = 0.10)) %>% mutate(inf = "p"))
...
dat_out_opt_alt = 
  ...
  filter(inf == "p") %>% 
  select(target, mor_m_i_cou, mor_m_p_cou, mor_m_a_cou, cos_i, cos_p) %>% 
...
ggsave("rff_whales_m_c_20231109.png", dpi = 300, width = 9, height = 6)
