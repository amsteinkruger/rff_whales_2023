...
dat_opt %>% write_csv("out/dat_opt.csv") # %>% select(-message) 
...
#  Save data for Table A1. 
dat_in %>% 
  group_by(year, month) %>% 
  summarize(mor_m_i_ref = sum(mor_m_i_ref),
            mor_m_p_ref = sum(mor_m_p_ref),
            mor_s_i_ref = sum(mor_s_i_ref),
            mor_s_p_ref = sum(mor_s_p_ref)) %>% 
  write_csv("out/dat_mor.csv")
