...
sta =
  ...
  filter(StateUSPS %in% c("CA", "OR", "WA")) %>%
  select %>%
  st_union %>%
  st_transform(crs = crs)

# Add rough masks for the Strait of Juan de Fuca and the Salish Sea (WA) and San Francisco Bay (CA).
...
sta = sta %>% st_union(mask_wa)

# ca_waters =
waters = 
  st_read("data/eez/FederalAndStateWaters.gdb") %>%
  filter(Jurisdicti %in% c("California", "Oregon", "Washington", "Federal")) %>%
  select %>%
  st_transform(crs = crs) %>%
  st_intersection(box %>%
...
rsd =
  ...
  st_join(.,
          box %>%
            ...
            st_transform(crs = crs) %>% 
            st_intersection(waters %>% st_buffer((25000^2 + 25000^2)^(1 / 2))) %>% # Intersect w/ EEZ + buffer to keep RSD points within polygons.
  ...
  full_join(box %>%
              ...
              st_transform(crs = crs) %>% 
              st_intersection(waters),
            by = "id") %>% # Get polygon geometries.
  st_sf %>% 
  st_intersection(waters) %>% # Trim buffer off. The buffer kept st_within from losing points outside EEZ describing areas within EEZ.
...
grid_rsd_buf = 
  ...
  st_transform(crs = crs) %>% 
  mutate(id = seq(1, nrow(.))) %>% # Get a unique numeric identifier for each polygon.
  st_intersection(waters %>% st_buffer(50000))
